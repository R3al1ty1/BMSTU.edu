...
namespace dbmsLib {
	...
	//	"DBDate"
	//};
	// Condition-перечисление условий, по которым могут сравниваються поля таблиц
	// БД:
	enum Condition {
		Undefined,
		Equal,
	...
	public:
		...
		//в таблице может быть несколько записей с одинаковым значением вторичного
		//ключа.
	...
	bool comparator(/*TableDataType*/ DBType type, void *obj1, Condition condition,
		void *obj);
	...
	public:
		...
		DBTableTxt(string tabName, Header hdr,
			string primKey); /*:DBTable(tabName,hdr,primKey){}*/
		~DBTableTxt(); //!!!деструктор перегружен,чтобы память,выделенная void*,не
		//!утекала
		...
		friend void ReadDBTable5(DBTableTxt &tab,
			string tabName); // tabName=path+tableName
		friend void WriteDBTable5(DBTableTxt &tab, string tabName);
		friend void PrintTable5(DBTableTxt &tab, int screenWidth);
	...
	class DBTableBin : public DBTable {
		...
		int DataBegin();
		int FieldPosition(
			string colName); //смещение поля colName от начала строки в байтах
		int FieldLength(string colName); //длина поля colName в байтах
		void ResizeData(int deltaRows);
		char **data;
		int maxRows; //число строк в data с учетом DELTA (при считывании
		//таблицы//????int maxRows()
	...
	public:
		...
		//в таблице может быть несколько записей с одинаковым значением вторичнего
		//ключа.
		...
		void SetHeader(Header &hdr);
		void
			ReadDBTable(string fileName); // fileName="..\\"+dbName+"\\"+tableName+".bin"
	...
	public:
		...
		friend int ReadDB5(DBTableSet &db1);
		friend void WriteDB5(DBTableSet &db1);
		friend void PrintDB5(DBTableSet &db1, int numcol);
	};
